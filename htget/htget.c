...
int sock_getline(int sock, char *buffer, int len)
{
	int		i;
	char		ch;

	for (i = 0, --len; i <= len && soread(sock, &ch, 1) > 0; )
	{
		if (ch == '\n')
			break;
		else if (ch != '\r')
...
int htget(char *host, int port, char *path)
{
	...
	if (verbose)
		(void)fprintf(stderr, "Getting IP address for %s\n", host);
	...
	if (password != 0)
	{
		...
		(void)fprintf(stderr, "%s => %s\n", password, buffer);
#endif
		i = sprintf(request, "Authorization: Basic %s\r\n", buffer);
		sowrite(sock, request, i);
	}
	...
	if ((contentlength = header(path)) > 0L && !headeronly)
	{
		...
		*/
		...
		length = 0L;
		while ((i = soread(sock, buffer, sizeof(buffer))) > 0)
		{
			(void)_write(output, buffer, i);
			length += i;
